...
const Task1 = ({task, index, openLabels, setOpenLabels, openEditModal, cardEdit, groupId}) => {
			...
			{(provided) => (
				<li
					...
					<div className='board-card-details'>
						{task.labels ? (
							<div className={openLabels ? 'card-labels-open flex' : 'card-labels flex'}>
								<ul>
						...
						<div className='card-title flex'>
							<Link to={`/board/${boardId}/${task.id}/${groupId}`}>{task.title}</Link>
							<div
							...
							<div className='card-icons flex'>
								{task.date ? (
									<div
										...
										<img src={task.date.isComplete || task.date.overDue ? dueDate : dueDateGray} />
										<span>{task.date.date}</span>
									</div>
								) : null}
								{task.description ? (
									<img className='board-card-description' src={description} title='this card has description' />
								) : null}
								{task.attachments ? (
									...
									task.users.map((user) => {
										return (
											<div key={user._id} className='card-members flex'>
												{user.imgUrl ? (
													<img src={user.imgUrl} alt={user.initials} title={user.fullname} />
												) : (
													<div className='flex align-center'>
														<div className='member-avatar-btn flex-center'>{user ? user.initials : 'G'}</div>
													</div>
												)}
											</div>
										);
