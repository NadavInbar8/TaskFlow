...
// Redux
import {loadBoard, addCard, updateBoard, openModal} from '../store/board.action.js';
...
export const Board = () => {
	...
	const addNewCard = (list) => {
		...
		let activity = `${loggedInUser.fullName} added a card in group ${list.title} at ${getNiceDate()}`;
	...
	const addNewGroup = () => {
		...
		setNewList(false);
		let activity = `${loggedInUser.fullName} added a Group in Board ${board.title} at ${getNiceDate()}`;
		updatedBoard.activities.push(activity);
		dispatch(updateBoard(updatedBoard));
		socket.emit('updateBoard', updatedBoard);
	};
	...
	const deleteList = async (list) => {
		const updatedBoard = {...board};
		const groupIdx = updatedBoard.groups.findIndex((group) => group.id === list.id);
		updatedBoard.groups.splice(groupIdx, 1);
		updatedBoard.groupsOrder.splice(groupIdx, 1);
		let activity = `${loggedInUser.fullName} deleted a Group in Board ${board.title} at ${getNiceDate()}`;
		updatedBoard.activities.push(activity);
		setForceRender(!forceRender);
	...
	const editCard = (list, card) => {
		...
		let activity = `${loggedInUser.fullName} edited a Card in Group ${list.title} at ${getNiceDate()}`;
		updatedBoard.activities.push(activity);
		dispatch(updateBoard(updatedBoard));
		socket.emit('updateBoard', updatedBoard);
	...
	const copyCard = (list, card) => {
		...
		let activity = `${loggedInUser.fullName} copied a Card in Group ${list.title} at ${getNiceDate()}`;
		updatedBoard.activities.push(activity);
		closeEditModal();
		dispatch(updateBoard(updatedBoard));
		socket.emit('updateBoard', updatedBoard);
	...
	const copyList = (list) => {
		...
		let activity = `${loggedInUser.fullName} copied a Group in Board ${board.title} at ${getNiceDate()}`;
		updatedBoard.activities.push(activity);
		closeListModal();
		dispatch(updateBoard(updatedBoard));
		socket.emit('updateBoard', updatedBoard);
	...
	const deleteCard = (list, card) => {
		const updatedBoard = {...board};
		let listIdx = updatedBoard.groups.findIndex((group) => group.id === list.id);
		let cardIdx = updatedBoard.groups[listIdx].tasks.findIndex((task) => task.id === card.id);
		...
		let activity = `${loggedInUser.fullName} deleted a Card in Group ${list.title} at ${getNiceDate()}`;
		updatedBoard.activities.push(activity);
		closeEditModal();
		setForceRender(!forceRender);
		dispatch(updateBoard(updatedBoard));
	...
	const onDragEnd = (result) => {
		...
		const startGroupIdx = data.groups.findIndex((fGroup) => fGroup.id === source.droppableId);
		const finishGroupIdx = data.groups.findIndex((fGroup) => fGroup.id === destination.droppableId);
		...
		if (groupStart === groupFinish) {
			...
			const startTaskIdx = newStart.tasks.findIndex((task) => task.id === draggableId);
							...
							<div className='flex' {...provided.droppableProps} ref={provided.innerRef}>
								{data
									? data.groupsOrder.map((groupId, index) => {
											const groupInx = data.groups.findIndex((sGroup) => sGroup.id === groupId);
