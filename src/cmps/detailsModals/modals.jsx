...
export function Members({board, members, loggedInUser, addUserToCard, toggleModal}) {
	...
	return (
		<div>
			{membersMinusLoggedInUser && (
				<div className='details-modal members' onClick={(ev) => ev.stopPropagation()}>
					...
					<main className='details-modal-content' onClick={(ev) => ev.stopPropagation()}>
							...
							<li
								...
								<div className='user-profile-div'>
									...
									{loggedInUser?.imgUrl ? (
										...
										<div className='user-details'>
											<div className='flex align-center'>
												<div className='user-avatar-btn flex-center'>{loggedInUser.initials}</div>
												<span>{loggedInUser.fullName}</span>
											</div>
											{/* <span>{checkUserInBoard(loggedInUser._id)}</span> */}
										</div>
									...
									<li
										...
										<div key={idx} className='user-profile-div'>
											...
											{member?.imgUrl ? (
												<div className='user-details'>
													<div className='flex align-center'>
														<img className='user-avatar-btn flex-center' src={member.imgUrl} alt='' />
												...
												<div className='user-details'>
													<div className='flex align-center'>
														<div className='user-avatar-btn flex-center'>{member.initials}</div>
														<span>{member.fullName}</span>
													</div>
													{/* <span>{checkUserInBoard(member._id)}</span> */}
												</div>
...
export function Checklist({toggleModal, addCheckList}) {
	...
	return (
		<div className='details-modal cheklist' onClick={(ev) => ev.stopPropagation()}>
			<section className='details-modal-top'>
			...
			<div className='details-modal-content' onClick={(ev) => ev.stopPropagation()}>
				<h4>Title</h4>
...
export function Attachment({attachLink, toggleModal}) {
	...
	return (
		<div className='details-modal attachment' onClick={(ev) => ev.stopPropagation()}>
			<section className='details-modal-top'>
			...
			<section className='details-modal-content' onClick={(ev) => ev.stopPropagation()}>
				<h4 className='add-img-from-computer'>
					Computer <input onChange={(event) => uploadImg(event)} type='file' />
				</h4>
				<h4>Attach a link</h4>
				<input placeholder='Attach any link here...' onChange={handleChange} name='link' value={link} type='text' />
				{nameInputShown && (
					<>
						<h4>Link name (optional)</h4>
						<input onChange={handleNameChange} name='name' value={name} type='text' />
					</>
				)}
				<button
...
export function EditAttachmentName({toggleModal, updateAttachmentName, idx, attachment}) {
	...
	return (
		<section
			...
			<main className='details-modal-content'>
				<h3>Change name:</h3>
				<input onChange={handleChange} name='name' value={name} onChang={handleChange} type='text' />
				<button
					onClick={(ev) => {
						ev.stopPropagation();
...
export function Cover({addCover, toggleModal}) {
	const covers = ['green', 'yellow', 'blue', 'dark-blue', 'purple', 'pink', 'orange', 'red', 'light-blue', 'turkiz'];
					...
					{covers.map((cover, idx) => {
						return (
							<div
								...
								key={idx}
								className={cover + '-cover' + ' ' + 'cover-to-show'}></div>
						);
					})}
				</section>
			</section>
...
export function Labels({addLabel, toggleModal, board, updateLabelsList}) {
	...
	return (
		<div className=' labels details-modal' onClick={(ev) => ev.stopPropagation()}>
			<section className='details-modal-top'>
				<img
					className='arrowleft'
					onClick={(ev) => {
...
export function Move({board, moveCardToOtherList, toggleModal, type}) {
	...
	return (
		<div className='details-modal move-modal' onClick={(ev) => ev.stopPropagation()}>
			{type === 'move' && (
				<section className='details-modal-top'>
					<span>Move</span>
					<img
			...
			<main className='details-modal-content'>
				...
				<section className='move-to'>
					<select onChange={handleListChange} className='move-to-list' name='list' id=''>
					...
					<select onChange={handlePositionChange} className='move-to-position' name='position'>
						...
						<option value={selectedGroup.tasks.length + 1}>{selectedGroup.tasks.length + 1}</option>
					</select>
				</section>
			</main>
...
export class Dates extends React.Component {
	...
	render() {
		return (
			<div className='details-modal Dates' onClick={(ev) => ev.stopPropagation()}>
				<section className='details-modal-top'>
					<span>Date</span>
					<img
						onClick={(ev) => {
