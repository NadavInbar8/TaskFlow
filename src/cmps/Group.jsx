							...
							{(provided) => (
								<>
									<ul
										...
										style={!selectedCard.id ? {overflowY: 'auto'} : {overflowX: 'visible', height: 'auto'}}>
										{group.tasks.length > 0
											? group.tasksIds.map((taskId, index) => {
													const taskObj = tasks.find((temp) => temp.id === taskId);
													return selectedCard.id !== taskObj.id ? (
														...
														<li key={taskObj.id} className='board-card overlaySee'>
															<textarea
																type='text'
																className='textarea-edit-card'
																onChange={handleChange}
																defaultValue={taskObj.title}></textarea>
															<div className='add-card' onClick={() => editCard(group, taskObj)}>
																save
															</div>
															<div className='edit-modal'>
																<ul>
																	<li>
																		<Link onClick={closeEditModal} to={`/board/${boardId}/${taskObj.id}/${group.id}`}>
																			Open Card
																		</Link>
																	</li>
																	<li>Edit Labels</li>
																	...
																	<li>Change Cover</li>
																	<li onClick={() => copyCard(group, taskObj)}>Copy</li>
																	<li>Edit Dates</li>
																	<li onClick={() => deleteCard(group, taskObj)}>Archive</li>
																</ul>
															</div>
														</li>
													);
									...
									{group.editMode ? (
										<div className='add-new-card-edit'>
											<textarea
												...
												name='newCard'
												onChange={handleChange}
												placeholder='Add card title...'></textarea>
											<div className='flex'>
												<div className='add-new-card-edit-btn' onClick={() => addNewCard(group)}>
													Add
												</div>
												<div
										...
										<div className='add-new-card' onClick={() => editNewCard(group)}>
						...
						{selectedList.id === group.id && modal === 'groupModal' ? (
							<div className='board-list-modal seeOverlay'>
								<div className=' modal-top'>
									<h3>Group Modal</h3>
									<img onClick={() => closeListModal(group)} className='closeBtn' src={close} alt='close' />
								</div>
								<ul>
